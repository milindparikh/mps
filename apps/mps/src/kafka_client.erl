...
-export ([start_link/0, start_link/1, request_metadata/2, response_metadata/2, request_produce/2, response_produce/2, request_fetch/2, response_fetch/2, request_offset/2, response_offset/2]).
...
-record(state, {socket,  leftover=nill, list_responses = [],
...
%    io:format("Data to be consumed  ~p~n", [State#state.leftover]),
	...
	true ->
	    ...
	    NewResponseList = lists:flatten([ListOfResponses | State#state.list_responses]),
